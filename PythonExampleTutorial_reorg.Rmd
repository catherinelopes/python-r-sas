...
The mean of numeric variables of a [DataFrame](#DataFrame) are available by calling the [mean()](https://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.mean.html) function on the data.
...
```
[PCA](http://scikit-learn.org/stable/modules/generated/sklearn.decomposition.PCA.html#sklearn.decomposition.PCA) | [np.transpose()](https://docs.scipy.org/doc/numpy-1.12.0/reference/generated/numpy.transpose.html)
...
# First we should scale the data, since R does
...
svMod.fit(train_scaled.drop(["Target"], axis = 1), 
                  train_scaled["Target"])
```
...
from sklearn.svm import SVC
svMod = SVC(random_state = 29, kernel = 'linear')
svMod.fit(train_scaled.drop(["Target"], axis = 1), train_scaled["Target"])
# Prediction on testing data
prediction = svMod.predict(test_scaled.drop(["Target"], axis = 1))

# Determine how many were correctly classified
Results = np.where(test_scaled["Target"] == prediction, "Correct", "Wrong")
print(pd.crosstab(index=Results, columns="count"))
```
...
scaler = StandardScaler().fit(np.array(train)) 
train_scaled = scaler.transform(np.array(train)) 
train_scaled = pd.DataFrame(train_scaled) 

scaler = StandardScaler().fit(np.array(test)) 
...
svMod = SVR()
svMod.fit(train_scaled.drop([13], axis = 1), train_scaled[13])
```
...
# Prediction on testing data
prediction = pd.DataFrame()
prediction["predY"] = svMod.predict(test_scaled.drop([13], axis = 1))

# Determine mean squared error
prediction["sq_diff"] = (test_scaled[13] - prediction["predY"])**2
print(prediction["sq_diff"].mean())
```
...
from sklearn.neural_network import MLPRegressor
nnMod = MLPRegressor(max_iter = 200, random_state = 29, solver = 'lbfgs')
nnMod.fit(train_scaled, train["Target"] / 50)
...
from sklearn.ensemble import RandomForestRegressor
rfMod = RandomForestRegressor(random_state=29)
rfMod.fit(train.drop(["tip"], axis = 1), train["tip"])

# Evaluation on training data
predY = rfMod.predict(train.drop(["tip"], axis = 1))

# Determine coefficient of determination score
from sklearn.metrics import r2_score
r2_rf = r2_score(train["tip"], predY)
...
from sklearn.metrics import r2_score
from sklearn.ensemble import RandomForestRegressor
rfMod = RandomForestRegressor(random_state=29, n_estimators = 500)
rfMod.fit(train.drop(["tip"], axis = 1), train["tip"])
from sklearn.metrics import r2_score
# Random Forest Regression Model (rfMod)

# Evaluation on testing data
predY = rfMod.predict(test.drop(["tip"], axis = 1))

# Determine coefficient of determination score
r2_rf = r2_score(test["tip"], predY)
print("Random forest regression model r^2 score (coefficient of determination): %f" % r2_rf)
```
...
features_mat = features.as_matrix()

msz0 = 1
msz1 = 150

sm = SOMFactory.build(features_mat, mapsize = [msz0, msz1], lattice = 'rect', initialization = 'random')

sm.train(verbose = False)

clust = sm.cluster(n_clusters = 3)

print(pd.crosstab(index = iris["Species"], columns = clust))
```
