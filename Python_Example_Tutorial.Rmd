...
clf = MLPClassifier(max_iter = 200, hidden_layer_sizes=(100,), random_state = 29)
model = clf.fit(train.drop(["Target"], axis = 1), train["Target"])

# Assess the model against the testing data
preds = clf.predict(test.drop(["Target"], axis = 1))

from sklearn.metrics import confusion_matrix
cm = pd.DataFrame(confusion_matrix(test["Target"], preds))
print(cm)
```
...
clf = MLPRegressor(max_iter = 200, random_state = 29, solver = 'lbfgs')
model = clf.fit(train_scaled, train["Target"] / 50)

# Assess against testing data, remembering to multiply by 50
preds = pd.DataFrame()
preds["predY"] = model.predict(test_scaled)*50
preds["target"] = test["Target"]
preds["sq_error"] = (preds["predY"] - preds["target"])**2
print(np.mean(preds["sq_error"]))
```
