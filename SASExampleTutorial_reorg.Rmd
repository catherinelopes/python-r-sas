...
/* Output of this model gives assessment against training data, a plot of 
	 the tree, and variable importance */
```
Output:
![output](DT11.png)
...
/* Output of this model gives assessment against training data, 
	 a plot of the tree, and variable importance */
```
...
Currently, there is not a gradient boosting procedure available in SAS, currently.  Therefore, the best method to create a gradient boosting model currently is using SAS Enterprise Miner.  Create the following diagram in SAS Enterprise Miner:
...
Output:
![output](Misclassification2.png)

### b) Fit a gradient boosting regression model.

Again, there is not a gradient boosting procedure available in SAS, currently.  Create the following diagram in SAS Enterprise Miner:
...
proc iml;
  submit / R;
    ...
    # Compute the mean of the squared differences (mean squared error) # as an assessment of the model
...
```{r}
proc import out = iris
  datafile = '/folders/myfolders/ANZ/iris.csv'
  dbms = csv replace;
  getnames = yes;
run;
...
proc iml;
  submit / R;
    ...
    labels <- dbscan$cluster table(labels, iris$Species)
  endsubmit;
quit;
```
