...
getnames = yes;
run;
proc contents data = student short;
run;
```
...
The [MEANS procedure](#means) prints the mean of numeric variables of a [SAS data set](#DATASET), as well as other descriptive statistics.
```{r, echo=6:8}
proc import out = student
datafile = 'C:/Users/class.csv'
dbms = csv replace;
...
getnames = yes;
run;
/* SAS uses a different method than Python and R to compute compute
...
The tables statement allows you to specify multiple variables at once, separated only by a space, so both of these tables could have been created with one [FREQ procedure](#freq) call.
...
getnames = yes;
run;
proc corr data = student pearson;
var Height Weight;
run; 
```
[CORR Procedure](#corr)
...
/* Also, SAS automatically plots the mean on the boxplot */
...
[REG Procedure](#reg), [DATA step](#step)([set](#set) statement), [macro variables](https://v8doc.sas.com/sashtml/macro/z1071889.htm), [call symput()](https://v8doc.sas.com/sashtml/macro/z0210266.htm) function
...
run;
```
[if-then/else](#if) & [output] statements, [do loop](#do), [end=](#step) & [firstobs=](http://support.sas.com/documentation/cdl/en/ledsoptsref/69751/HTML/default/viewer.htm#p0wjxoxrco6dsgn1ls5n3mbybcng.htm) data set options
...
/* Output of this model gives assessment against training data, a plot of the 
	 tree, and variable importance */
```
Output:
![output](DT11.png)
...
proc hpforest data = train;
	input sepal_length__cm_ sepal_width__cm_ petal_length__cm_ 
	      petal_width__cm_ / level = interval;
...
### Build a model and assess the model against the training & testing data.

Currently, there is not a Gradient Boosting Procedure available in SAS.  Therefore, the best method to create a gradient boosting model currently is using SAS Enterprise Miner.  Create the following diagram in SAS Enterprise Miner:

Output:
![output](EM1.png)

This diagram results in the following misclassification against training & testing data:

Output:
![output](Misclassification1.png)

## 5.9 Fit a support vector classification model.

### Build a model and assess the model against the training & testing data.
```{r} 
/* Notice we are re-using data sets but it is good to re-read the original version back into the 
   environment */
proc import out = train
	datafile = 'C:/Users/breastcancer_train.csv'
	dbms = csv replace;
...
run;
```
[HPSVM Procedure](#svm), [%include](#include) & [if-then/else](#if) statements, [FREQ Procedure](#freq)
...
/* Finally we can assess the model against the 
	 test data */
data scored;
	set gridout;
	correct = (Target = I_Target);
...
data scored(keep = Target prediction correct);
	set scored;
	if (P_Target1 <= 0.5) then prediction = 0;
	else prediction = 1;
	correct = (Target = prediction);
...
proc iml;
	...
	accuracy_tree = (1/nrow(data)) * sum(data[,3]);
...
quit; 
```
Again, recall that there is not a Gradient Boosting Procedure available in SAS.  Therefore, the best method to create a gradient boosting model currently is using SAS Enterprise Miner.  Create the following diagram in SAS Enterprise Miner:
...
This implies the accuracy score, which is 1 - misclassification is 

Training Accuracy Score: 1 - 0.01005 = 0.98995

Testing Accuracy Score: 1 - 0.02924 = 0.97076

[HPSPLIT Procedure](#hpsplit), [%include](#include) & [if-then/else](#if) statements, [IML Procedure](#iml), [HPFOREST Procedure](#hpforest), [HP4SCORE Procedure](#hp4score)

## 6.3 Evaluation with cross validation.

### a) KFold

The HPFOREST output for the 5 models has been removed from the output in this tutotiral for the sake of space.  Unfortunately the HPFOREST Procedure does no have a "noprint" option.
```{r}
...
data folds;
	...
	*randomly assign observation to one of K groups;
	call streaminit(29);
 	...
 	output;
run;
/* Just for validation purposes, no pun intended */
proc freq data = folds;
tables rand;
run;
...
proc surveyselect data = breastcancer out = cv seed = 29 samprate = 0.7 outall reps = 5;
run;
...
### [COMPARE Procedure](http://support.sas.com/documentation/cdl/en/proc/65145/HTML/default/viewer.htm#n0c1y14wyd3u7yn1dmfcpaejllsn.htm)  {#compare} 

### [CONTENTS Procedure](http://support.sas.com/documentation/cdl/en/proc/65145/HTML/default/viewer.htm#n1hqa4dk5tay0an15nrys1iwr5o2.htm) {#contents}

### [CORR Procedure](http://support.sas.com/documentation/cdl/en/procstat/66703/HTML/default/viewer.htm#procstat_corr_overview.htm) {#corr}

### [FCMP Procedure](http://support.sas.com/documentation/cdl/en/proc/65145/HTML/default/viewer.htm#n1aozmc89vjkpzn1q6a54nleh56o.htm) {#fcmp}

### [EXPORT Procedure](http://support.sas.com/documentation/cdl/en/proc/70377/HTML/default/viewer.htm#p09k160vk93xxhn171zz1z6551w2.htm) {#export}

### [FREQ Procedure](https://support.sas.com/documentation/cdl/en/statug/63962/HTML/default/viewer.htm#statug_freq_sect006.htm) {#freq}

### [GENMOD Procedure](https://support.sas.com/documentation/cdl/en/statuggenmod/61787/PDF/default/statuggenmod.pdf) {#genmod}

### [HP4SCORE Procedure](https://support.sas.com/documentation/onlinedoc/miner/em14/emhpprcref.pdf) {#hp4score}

### [HPFOREST Procedure](https://support.sas.com/documentation/onlinedoc/miner/em14/emhpprcref.pdf) {#hpforest}

### [HPSPLIT Procedure](http://support.sas.com/documentation/cdl/en/stathpug/66410/HTML/default/viewer.htm#stathpug_hpsplit_overview.htm) {#hpsplit}

### [HPSVM Procedure](http://documentation.sas.com/?docsetId=emhpprcref&docsetVersion=14.2&docsetTarget=emhpprcref_hpsvm_overview.htm&locale=en) {#hpsvm}

### [IML Procedure](https://support.sas.com/documentation/cdl/en/imlug/63541/PDF/default/imlug.pdf) {#iml}

### [IMPORT Procedure](http://support.sas.com/documentation/cdl/en/acpcref/69731/HTML/default/viewer.htm#p0jf3o1i67m044n1j0kz51ifhpvs.htm) {#import}

### [MEANS Procedure](http://support.sas.com/documentation/cdl/en/proc/65145/HTML/default/viewer.htm#p0f0fjpjeuco4gn1ri963f683mi4.htm) {#means}

### [PRINCOMP Procedure](https://support.sas.com/documentation/cdl/en/statug/63347/HTML/default/viewer.htm#princomp_toc.htm) {#princomp}

### [PRINT Procedure](http://support.sas.com/documentation/cdl/en/proc/65145/HTML/default/viewer.htm#p10qiuo2yicr4qn17rav8kptnjpu.htm) {#print}

### [REG Procedure](https://support.sas.com/documentation/cdl/en/statug/63033/HTML/default/viewer.htm#statug_reg_sect007.htm) {#reg}

### [SCORE Procedure](https://support.sas.com/documentation/cdl/en/statug/63347/HTML/default/viewer.htm#statug_score_sect001.htm) {#score}

### [SGPLOT Procedure](http://support.sas.com/documentation/cdl/en/grstatproc/69716/HTML/default/viewer.htm#p1t32i8511t1gfn17sw07yxtazad.htm) {#sgplot}
...
### [SGSCATTER Procedure](http://support.sas.com/documentation/cdl/en/grstatproc/62603/HTML/default/viewer.htm#sgscatter-syn.htm) {#sgscatter}

### [SORT Procedure](http://support.sas.com/documentation/cdl/en/proc/70377/HTML/default/viewer.htm#p02bhn81rn4u64n1b6l00ftdnxge.htm) {#sort}

### [SQL Procedure](https://support.sas.com/documentation/cdl/en/sqlproc/69822/HTML/default/viewer.htm#n1oihmdy7om5rmn1aorxui3kxizl.htm) {#sql}

### [SURVEYSELECT Procedure](https://support.sas.com/documentation/cdl/en/statug/63347/HTML/default/viewer.htm#statug_surveyselect_sect001.htm) {#surveyselect}
...
### [%include](http://support.sas.com/documentation/cdl/en/lrdict/64316/HTML/default/viewer.htm#a000214504.htm) {#include} statement

### [if-then/else](http://support.sas.com/documentation/cdl/en/lrdict/64316/HTML/default/viewer.htm#a000202239.htm) {#if} statement

### [infile](http://support.sas.com/documentation/cdl/en/lrdict/64316/HTML/default/viewer.htm#a000146932.htm) {#infile} statement

### [input](http://support.sas.com/documentation/cdl/en/lrdict/64316/HTML/default/viewer.htm#a000146292.htm) {#input} statement

### [merge](http://support.sas.com/documentation/cdl/en/lrdict/64316/HTML/default/viewer.htm#a000202970.htm) {#merge} statement

### [output](http://support.sas.com/documentation/cdl/en/lrdict/64316/HTML/default/viewer.htm#a000194540.htm) {#output} statement

### [set](http://support.sas.com/documentation/cdl/en/lrdict/64316/HTML/default/viewer.htm#a000173782.htm) {#set} statement

### [where](http://support.sas.com/documentation/cdl/en/lrdict/64316/HTML/default/viewer.htm#a000202951.htm) {#where} statement

\newpage

# Alphabetical Index

## [CHAR](http://support.sas.com/documentation/cdl/en/fedsqlref/67364/HTML/default/viewer.htm#n19bf2z7e9p646n0z224cokuj567.htm) {#char}
...
## [DOUBLE](http://support.sas.com/documentation/cdl/en/fedsqlref/67364/HTML/default/viewer.htm#n19bf2z7e9p646n0z224cokuj567.htm) {#double}
