...
```{r}
# Prediction on testing data
pred.response <- unname(predict(svMod, test))
...
#### i) Generate random data based on a sine curve.
```{r}
set.seed(29)
...
[seq()](https://www.rdocumentation.org/packages/base/versions/3.4.0/topics/seq) [runif()](https://www.rdocumentation.org/packages/stats/versions/3.4.0/topics/Uniform)

#### ii) Fit a support vector regression model to the data and assess the predictions.
```{r, message = FALSE, warning = FALSE}
# The package we need to fit an svm model (e1071) must first 
# be installed and then called to use
library(e1071)
