...
Before beginning this tutorial, the packages listed in the README file should be downloaded and installed properly.

In R, comments are indicated in code with a "#" character, and arrays and matrices begin with index 1.
...
There is more code involved in reading a .json file into R so it becomes a proper data frame, however we will not at this time dive into the explanation for all this code, but it should become evident throughout the tutorial.
```{r, eval = FALSE}
...
train2 <- read.csv("/Users/iris_train.csv")
test2 <- read.csv("/Users/iris_test.csv")
...
var_import <- data.frame(sort(var_import, decreasing = TRUE, index.return = TRUE))
...
### a) Fit a neural network regression model.
...
```{r, warning = FALSE, message = FALSE}
a_fit <- arima(air_series, order = c(0,1,1), seasonal = list(order = c(0,1,1), period = 12), method = "ML")
...
```{r}
plot(a_fit$residuals, xlab = "Month", ylab = "Residuals", type = 'h')
```
...
Workng with [data frame](#DataFrame) like objects
